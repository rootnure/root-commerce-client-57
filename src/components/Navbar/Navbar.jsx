...
const Navbar = () => {
    ...
    <>
      <NavLink
        ...
        className={`root-nav-btn ${
          isDarkMode === "dark" ? "border-white" : "border-black"
        }`}>
        <IoMdHome></IoMdHome>
      </NavLink>
      {user ? (
        <>
          ...
          <div
            className="root-nav-btn !border-0 !px-0 hover:!bg-transparent flex dropdown dropdown-end"
            title={`Profile of ${user.displayName}`}>
            <label
              tabIndex={0}
              className="btn bg-transparent hover:bg-transparent pl-0 pr-0 border-0 h-auto min-h-fit">
              <img
                src={user.photoURL}
            ...
            <ul className="p-2 shadow menu dropdown-content z-[1] rounded-box w-52 border mt-8 bg-white dark:bg-black text-orange-600">
              <li className="font-bold text-center text-black dark:text-orange-600">
                {user?.displayName}
              </li>
              <li className="text-center text-gray-400 mt-2 mb-1">
                {user?.email}
              </li>
              <li>
                <NavLink
                  to="/profile"
                  title="My Profile"
                  className="text-center block hover:font-semibold hover:text-orange-600">
                  My Profile
                </NavLink>
              </li>
            </ul>
          ...
          <button
            ...
            title="logout"
            className="root-nav-btn border-orange-600 dark:border-white">
  ...
  return (
    <>
      <header
        className={`bg-cover flex justify-between px-6 md:px-12 items-center ${
          isDarkMode === "dark" ? "bg-nav-dark" : "bg-nav-light"
        }`}>
        <section>
          ...
          <div className="flex items-center">
            <label className="swap swap-rotate">
              ...
              <IoMdSunny
                ...
                title="Dark"
                className="swap-on fill-current h-8 w-8 p-1.5 rounded-full bg-white text-orange-600"></IoMdSunny>
              <IoMdMoon
                onClick={() => setIsDarkMode("light")}
                title="Light"
                className="swap-off fill-current h-8 w-8 p-1.5 rounded-full bg-gray-600 text-white"></IoMdMoon>
            </label>
          </div>
        </section>
      </header>
